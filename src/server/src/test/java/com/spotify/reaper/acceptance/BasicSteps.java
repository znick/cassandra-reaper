...
public final class BasicSteps {
  ...
  public synchronized static void addReaperRunner(ReaperTestJettyRunner runner) {
  ...
  public synchronized static void removeReaperRunner(ReaperTestJettyRunner runner) {
  ...
  private static void callAndExpect(
      ...
      RUNNERS.parallelStream().forEach(runner -> {
            ...
            if (1 == RUNNERS.size() && expectedStatuses[0].getStatusCode() != response.getStatus()) {
                ...
                // duplicate http requests and the wrong request responds first
                LOG.error(
  ...
  public void the_last_added_repair_run_is_deleted_for_cluster_called() throws Throwable {
    ...
    synchronized (BasicSteps.class) {
        ...
        callAndExpect(
                ...
                Optional.of(params),
                Optional.of("\"" + TestContext.TEST_CLUSTER + "\""),
                Response.Status.OK, Response.Status.NOT_FOUND, Response.Status.FORBIDDEN);
